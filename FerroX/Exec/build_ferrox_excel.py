#!/usr/bin/env python3
"""
Build an Excel dataset index for FerroX simulations (MFIS, MFIM, MFM, ...).

Output design:
  1. summary          : generation metadata
  2. experiments      : one row per simulation folder
...
Default MFIS folder layout expected:
  ./MFIS*/inputs
  ./MFIS*/MFIS_PV_curve.csv or somewhere below MFIS*/
  ./MFIS*/plts/plt########
  ./MFIS*/run.log or ./MFIS*/plts/run.log

For MFIM, use ``--structure MFIM``.  The structure label, folder pattern,
metadata filename, P-V CSV filename, output workbook, and NPZ directory can
all be overridden from the command line.
...
def read_inputs(path: Path, wanted_params: list[str]) -> dict:
    params = {}
...
def read_run_log(folder: Path, run_log_filename: str = "run.log"):
    logfile = folder / run_log_filename
    if not logfile.exists():
        logfile = folder / "plts" / run_log_filename
    if not logfile.exists():
...
def read_pv_curve(
    ...
    sweep_branches: list[str],
    pv_filename: str,
    p_column: str | None = None,
    voltage_column: str | None = None,
):
    csv_path = find_first_file(folder, pv_filename)
    if csv_path is None:
        return None, None, f"{run_id}: {pv_filename} not found"
    ...
    if p_column is not None:
        p_col = p_column if p_column in df_raw.columns else None
    else:
        p_col = find_column(
            df_raw,
            ["P_mean", "Pavg", "P_average", "polarization", "P"],
        )

    if voltage_column is not None:
        vapplied_col = voltage_column if voltage_column in df_raw.columns else None
    else:
        vapplied_col = find_column(
            df_raw,
            [
                "V_applied",
                "Vapp",
    ...
    if p_column is not None and p_col is None:
        return None, str(csv_path), (
            f"{run_id}: requested --p-column {p_column!r} is absent from "
            f"{csv_path}. Columns = {list(df_raw.columns)}"
        )

    if voltage_column is not None and vapplied_col is None:
        return None, str(csv_path), (
            f"{run_id}: requested --voltage-column {voltage_column!r} is "
            f"absent from {csv_path}. Columns = {list(df_raw.columns)}"
        )

    # The original script required a Vg_mean column although it never wrote or
    # otherwise used that column.  MFM CSV files commonly do not have Vg_mean,
    # so only the polarization column is mandatory here.
    if p_col is None:
        return None, str(csv_path), (
            f"{run_id}: cannot identify the polarization column in {csv_path}. "
...
def build_dataset(args):
    ...
    folder_glob = args.folder_glob or f"{structure}*"
    pv_filename = args.pv_filename or f"{structure}_PV_curve.csv"
    out_xlsx = Path(args.out or f"{structure}_dataset.xlsx").resolve()
    pz_dir = Path(args.pz_dir or "extracted_pz").resolve()
    out_xlsx.parent.mkdir(parents=True, exist_ok=True)
    pz_dir.mkdir(parents=True, exist_ok=True)

    wanted_params = list(
        dict.fromkeys([*WANTED_PARAMS, *args.extra_param])
    )
    ...
    folders = sorted([p for p in root.glob(folder_glob) if p.is_dir()])
    if not folders:
        warnings.append(
            f"No simulation folders matched {folder_glob!r} under {root}"
        )

    for folder in folders:
        run_id = folder.name
        inp = find_first_file(folder, args.inputs_filename)
        if inp is None:
            warnings.append(
                f"{run_id}: {args.inputs_filename} not found; skipped"
            )
            continue

        print(f"Processing {run_id}")
        params = read_inputs(inp, wanted_params)
        ...
        start_time, end_time, elapsed_hms, elapsed_sec = read_run_log(
            folder,
            run_log_filename=args.run_log_filename,
        )

        pv_df, pv_csv_path, pv_warning = read_pv_curve(
            folder=folder,
            run_id=run_id,
    ...
    summary_rows = [
        ...
        {"item": "Folder glob", "value": folder_glob},
        {"item": "Inputs filename", "value": args.inputs_filename},
        {"item": "Run-log filename", "value": args.run_log_filename},
        {"item": "P-V CSV filename", "value": pv_filename},
        {"item": "Total simulation folders", "value": len(folders)},
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Build a FerroX Excel dataset index (MFIS, MFIM, MFM, ...)"
    )
    parser.add_argument(
        "--structure",
        "--device",
        ...
        default="MFIS",
        help=(
            "Structure label used by defaults. For example, --structure MFIM "
            "gives folder glob MFIM*, CSV name MFIM_PV_curve.csv, and output "
            "MFIM_dataset.xlsx. --device is a backward-compatible alias"
        ),
    )
    parser.add_argument(
        "--root",
        default=".",
        help="Root folder containing the simulation folders",
    )
    parser.add_argument(
        "--folder-glob",
        default=None,
        help="Simulation-folder glob; default is <structure>*",
    )
    parser.add_argument(
        "--inputs-filename",
        default="inputs",
        help="Inputs/metadata basename searched recursively; default is inputs",
    )
    parser.add_argument(
        "--run-log-filename",
        default="run.log",
        help="Run-log basename checked in each case and its plts folder",
    )
    parser.add_argument(
        "--pv-filename",
        default=None,
        help="P-V CSV basename searched recursively; default is <structure>_PV_curve.csv",
    )
    parser.add_argument(
        "--out",
        default=None,
        help="Output Excel filename; default is <structure>_dataset.xlsx",
    )
    parser.add_argument(
        "--pz-dir",
        default=None,
        help="Folder for compressed Pz .npz outputs; default is extracted_pz",
    )
    parser.add_argument(
        "--p-column",
        default=None,
        help="Exact polarization column name; otherwise it is detected automatically",
    )
    parser.add_argument(
        "--voltage-column",
        default=None,
        help=(
            "Exact applied-voltage column name; otherwise it is detected or "
            "reconstructed from --vmin/--vmax/--dv"
        ),
    )
    parser.add_argument(
        "--extra-param",
        ...
        default=[],
        metavar="NAME",
        help="Additional inputs parameter to store; may be repeated",
    )
